...
class ResnetGenerator(gluon.Block):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect'):
        assert(n_blocks >= 0)
...
class UnetGenerator(gluon.Block):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 norm_layer=nn.BatchNorm, use_dropout=False, gpu_ids=[]):
...
class NLayerDiscriminator(gluon.Block):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm, use_sigmoid=False, gpu_ids=[]):
        ...
        sequence.add(nn.Conv2D(ndf, kernel_size=kw, stride=(2,2), padding=padw))
        sequence.add(nn.LeakyReLU(0.2))
        ...
        for n in range(1, n_layers):
            ...
            nf_mult = min(2**n, 8)
            sequence.add(nn.Conv2D(ndf * nf_mult, kernel_size=kw, stride=(2,2), padding=padw))
        ...
        sequence.add(nn.Conv2D(ndf * nf_mult, kernel_size=kw, stride=(1,1), padding=padw))
        sequence.add(norm_layer())
        sequence.add(nn.LeakyReLU(0.2))

        sequence.add(nn.Conv2D(1, kernel_size=kw, stride=(1,1), padding=padw))

        if use_sigmoid:
            sequence.add(nn.Sigmoid())
